...
class DaangnSpider(BaseCollector):
    ...
    def search_url(self, query: str, *, only_on_sale: bool = True) -> str:
        ...
        return f"{base}?{urlencode({'in': self.region_in, 'q': query, 'only_on_sale': str(only_on_sale).lower()})}"
        ...
        for art in cards[:limit]:
            ...
            out.append(
                RawListing(
                    ...
                    description_text=f"{title} {region}\n\n--- .flea-market-article ---\n{excerpt}\n\n--- page ---\n{page_desc}",
                    trade_state=None,
...
if __name__ == "__main__":
    ...
    sp = DaangnSpider(stealth=bool(args.stealth), use_auto_wait=bool(args.stealth))
